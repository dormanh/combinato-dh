...
FIGSIZE = (13, 7.5)
GRID = GridSpec(
    2, 6, left=0.08, right=0.95, top=0.9, bottom=0.05, wspace=0.7, hspace=0.25
)
...
def plot_group(gid, group, group_joined, start_stop, sign, savefolder, thresholds):
    """
    ...
    plot.text(
        0.5,
        1.05,
        "Group {} ({}) Firing over time".format(gid, TYPE_NAMES[group_joined["type"]]),
        va="bottom",
        ha="center",
    ...
    # other density
    data = np.array([np.histogram(row, bins=DENSITY_BINS)[0] for row in spikes.T])
...
def run_file(fname, sign, label, savefolder):
    ...
    print(
        "Sorting contains {} spikes from {}, duration {}".format(nspk, entity, dur_str)
    )
    ...
    for gid in groups_joined:
        ...
        outfname = "{}_{}_{}_{:03d}_{}.png".format(
            bname, entname, label, gid, gtypename
        )
