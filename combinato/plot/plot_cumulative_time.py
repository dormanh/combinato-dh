...
def spike_cumulative(plot, times, special, ylim=None, text=True, lw=1, alpha=0.7):
    """
    ...
    plot.plot(times, np.linspace(ylim[0], ylim[1], len(times)), "m", lw=lw, alpha=alpha)
    ...
    if special:
        ...
        plot.text(xlim[1], ylim[0], nspk_str, ha="right", va="bottom", size=TEXT_SIZE)
