...
class GroupOverviewFigure(MplCanvas):
    ...
    def __init__(self, parent, width=5, height=5):
        ...
        grid = GridSpec(
            3, 3, left=0.05, right=0.97, top=0.98, bottom=0.05, hspace=0.15, wspace=0.2
        )
        ...
        for t in titles:
            t[0].text(
                ...
                1,
                t[1],
                transform=t[0].transAxes,
                backgroundcolor="w",
                horizontalalignment="right",
                verticalalignment="top",
            )
        self.cumSpikeAx.text(
            0,
            1,
        ...
        args = (
            (self.meanAx, 0, 1, "µV", "left", "top"),
            (self.maxDistrAx, 1, 0, "µV", "right", "bottom"),
            (self.overTimeAx, 1, 0, "min", "right", "bottom"),
            (self.overTimeAx, 0, 1, "µV", "left", "top"),
            (self.isiAx, 1, 0, "ms", "right", "bottom"),
            (self.cumSpikeAx, 1, 0, "min", "right", "bottom"),
        )
    ...
    def setOptions(self, xax, times, sign="pos", thresholds=None):
        ...
        self.isiAx.set_xticks(np.linspace(0, options["compute_isi_upto_ms"], 6))
    ...
    def updateInfo(self, group):
        ...
        if len(data) > 1:
            ...
            self.isiAx.cla()
            n, _, _ = self.isiAx.hist(
                data, nBins, color=options["histcolor"], histtype=options["histtype"]
            )
            self.isiAx.set_ylim((0, np.max(n) + 5))
            # mark percentage
            too_short = (data <= options["isi_too_short_ms"]).sum() / group.times.shape[
                0
            ]
        ...
        if len(times):
            ...
            self.maxDistrAx.cla()
            ns, _, _ = self.maxDistrAx.hist(
                mdata, 100, color=options["histcolor"], histtype=options["histtype"]
            )
        ...
        if len(data):
            self.meanDensAx.imshow(
                data, cmap=options["cmap"], aspect="auto", origin="lower"
            )
            self.meanDensLogAx.imshow(
                np.log(1 + data), cmap=options["cmap"], aspect="auto", origin="lower"
            )
    ...
    def mark(self, index, histdata=None):
        ...
        if histdata is not None:
            ...
            if len(histdata):
                n, _, self.additionalIsiPatches = self.isiAx.hist(
                    histdata, 100, color="r", histtype="stepfilled"
                )
...
class ComparisonFigure(MplCanvas):
    def __init__(self, parent, width=5, height=5):
        ...
        self.positions = ((0.1, 0.7, 0.2, 0.2), (0.7, 0.7, 0.2, 0.20))
    ...
    def xcorr(self, group1, group2):
        ...
        if lags.any():
            ...
            lags_plot = self.fig.add_axes((0.05, 0.05, 0.9, 0.9))
            ...
            # plot it
            bins = np.linspace(
                -lag, lag, min(2 * lag, max(2 * lag, np.sqrt(greater_len)))
            )
            hist_data, _ = np.histogram(lags, bins)
            lags_plot.bar(
                bins[:-1], hist_data, linewidth=0.1, width=(bins[1] - bins[0])
            )  # , histtype=options['histtype'])
            bigleg = bins[hist_data.argmax()]
            lags_plot.text(
                bigleg, hist_data.max(), "{:.0f} ms".format(bigleg), va="bottom"
            )
            ...
            for i, group in enumerate((group1, group2)):
                ...
                group_plot.patch.set_alpha(0.5)
                ...
                group_plot.text(
                    0.01,
                    1.1,
                    "{} ({})".format(group.name, TYPE_NAMES[group.group_type]),
                    transform=group_plot.transAxes,
                    va="bottom",
                )
...
class AllGroupsFigure(MplCanvas):
    ...
    def mark(self, groupName, index):
        ...
        data = self.session.groupsByName[groupName].clusters[index].meanspike
        ...
        for ax in self.fig.get_axes():
            ...
            ax.add_line(line)
            self.texts.append(
                ax.text(0.05, 0.05, format(dist, ".2f"), transform=ax.transAxes)
            )
