...
def plot_convolution(plot, rows, lw=1):
    ...
    smooth -= 0.5
    plot.plot(fr_bins, smooth[fr_w:-fr_w], "m", alpha=0.5, lw=lw)
...
def create_raster_rows(cl_times, onset_times):
    ...
    for onset_time in onset_times:
        idx = (cl_times >= onset_time - T_PRE) & (cl_times <= onset_time + T_POST)
        if idx.any():
            do_plot = True
...
def plot_one_plot(
    plot, scale, spike_times, onset_times, stim_name, image, scrtype, stimnum=None
):
    """
    plot either image or name screening raster plot into plot
    """
    colors = ((0, 0, 0.8), (0.7, 0, 0), (1, 1, 0))

    merged_spike_times = np.hstack(spike_times)
    merged_spike_times.sort()

    for i_sp, sptimes in enumerate(spike_times):
        rows, do_plot = create_raster_rows(sptimes, onset_times.copy())
        if do_plot:
            plot_one_cluster_one_stim(
                plot, rows, ylim=len(rows) + 1, color=colors[i_sp]
            )
        # else:
        #    print('Not plotting raster!')

    rows, do_plot = create_raster_rows(merged_spike_times, onset_times.copy())
    if do_plot:
...
def get_onset_times(frame, stimulus, daytime, paradigm):
    idx = (frame["stim_num"] == stimulus) & (frame["paradigm"] == paradigm)
    if len(daytime):
        ...
        for istim, stimulus in enumerate(stimuli):
            ...
            for shift, paradigm in iterator:
                if paradigm in set(self.frame.paradigm):
                    ...
                    onset_times = get_onset_times(
                        self.frame, stimulus, daytime, paradigm
                    )
                    if do_numbers:
                        number = stimulus + 1
                    else:
