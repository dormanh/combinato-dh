...
def main():
    ...
    parser.add_argument("--files", nargs="+", help=".ncs files to be extracted")
    ...
    parser.add_argument(
        "--jobs", nargs=1, help="job file contains one filename per row"
    )
    parser.add_argument("--matfile", nargs=1, help="extract data from a matlab file")
    parser.add_argument(
        "--h5",
        action="store_true",
        default=False,
    ...
    parser.add_argument(
        "--matfile-scale-factor",
        nargs="?",
        type=float,
        help="rescale matfile data by this factor" " (to obtain microvolts)",
        default=1,
    )
    parser.add_argument(
        "--destination", nargs=1, help="folder where spikes should be saved"
    )
    parser.add_argument(
        "--refscheme",
        nargs=1,
    ...
    if (args.files is None) and (args.matfile is None) and (args.jobs is None):
