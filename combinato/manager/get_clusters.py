...
def get_data_from_sessions(
    sorting_man, sessions, sign="pos", items=None, skip_artifacts=True, stack=True
):
    """
    retrieve data from a given list of sessions
    """
    groups, all_min, all_max = sorting_man.clusters_from_sessions(
        sessions, sign, skip_artifacts, stack
    )
    ...
    if "spikes" in items:
        data["spikes"] = sorting_man.get_h5data(sign).spikes[all_min : all_max + 1]
...
def get_times_from_sessions(sorting_man, sessions, sign="pos", skip_artifacts=True):
    """
    retrieve index first, then times. This is just a renamed function.
    """
    return get_data_from_sessions(
        sorting_man, sessions, sign, ["times"], skip_artifacts
    )
...
def test(fname):
    ...
    groups_full = get_data_from_sessions(
        sorting_man, ses_names, "pos", items=["spikes", "times"]
    )
    print(groups)
    print(groups_full)
