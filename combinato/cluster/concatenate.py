...
def write_sorting_file(h5fname, sorted_index, sorted_info, artifacts):
    ...
    fid.create_array("/", "distance", np.zeros(sorted_index.shape[0], np.float32))
...
def total_match(fid, all_spikes):
    ...
    print("classes: {}, all_spikes: {}".format(classes.shape, all_spikes.shape))
    ...
    for start, stop in zip(starts, stops):
        ...
        print("Calculating match for {} spikes".format(all_spikes[this_idx].shape[0]))
...
def plot_all_classes(classes, matches, all_spikes, plot_dirname):
    ...
    for clnum, clid in enumerate(clids):
        ...
        print(
            "Plotting {}/{}, {}, {} spikes".format(
                clnum + 1, len(clids), plot_dirname, cl_idx.sum()
            )
        )
        plot_class(fig, xax, xlim, ylim, all_spikes[cl_idx], matches[cl_idx], outname)
...
def parse_args():
    ...
    if args.jobs is None:
        if None in (args.datafile, args.sessions, args.label):
            raise ArgumentError(
                args.jobs, "Specify either jobs or datafile, " "sessions, and label"
            )
        else:
            outfname = main(args.datafile[0], args.sessions, args.label[0], do_plots)
            if do_groups:
                if outfname is not None:
        ...
        if single:
            ...
            pool.map(
                multi_helper,
                [
                    (fname, sessions, label, do_groups, do_plots)
                    for fname, sessions in jobdict.items()
                ],
            )
