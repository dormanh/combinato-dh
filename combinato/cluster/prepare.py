...
def make_arguments(
    filename, sign, mode, start=0, stop=None, max_nspk_session=20000, add_one=False
):
    """
    ...
    if mode == "time":
        times = h5manager.get_data_by_name_and_index(
            "times", non_artifact_idx, sign=sign
        )[:]
...
def main(
    fnames, sign, mode, start, stop, max_nspk_session, label, replace, add_one=False
):
    """
    ...
    for name in fnames:
        jobs = make_arguments(name, sign, mode, start, stop, max_nspk_session, add_one)
        if jobs is None:
            continue
...
def parse_arguments():
    ...
    parser.add_argument("--times", nargs="+", help="supply timestamps from files")
    parser.add_argument("--jobs", nargs=1, help="supply list of clustering jobs")
    parser.add_argument("--datafile", nargs=1, help="cluster from one datafile")
    parser.add_argument(
        "--between",
        default=False,
        action="store_true",
    ...
    parser.add_argument(
        "--neg", default=False, action="store_true", help="use negative spikes"
    )
    parser.add_argument(
        "--max-nspk", default=20000, type=int, help="maximal number of spikes per run"
    )
    parser.add_argument(
        "--label", default=getuser()[:3], help="name under which sorting is stored"
    )
    parser.add_argument("--start", nargs=1, type=int, help="start at this spike")
    parser.add_argument("--stop", nargs=1, type=int, help="stop at this spike")
