...
def cluster_step(features, folder, sub_name, overwrite):
    ...
    res_idx, tree, used_points = features_to_index(
        features, folder, sub_name, overwrite
    )
...
def iterative_sorter(features, spikes, n_iterations, name, overwrite=True):
    ...
    for i in range(n_iterations):
        ...
        if sub_idx.sum() < options["MinInputSize"]:
            if options["Debug"]:
                print("Stopping iteration, {} spikes left".format(sub_idx.sum()))
            break
        ...
        if options["Debug"]:
            print("Iteration {}, new classes: {}".format(i, np.unique(res_idx)))
        ...
        if options["ReclusterClusters"]:
            ...
            for clid in clids:
                ...
                if cluster_size < options["MinInputSizeRecluster"]:
                    if options["Debug"]:
                        print(
                            "Not reclustering cluster {} ({} spikes)".format(
                                clid, cluster_size
                            )
                        )
                    continue
                    ...
                    if options["Debug"]:
                        print(
                            "Reclustering cluster {} ({} spikes)".format(
                                clid, cluster_size
                            )
                        )
                ...
                recluster_res_idx = cluster_step(
                    features[recluster_idx], name, sub_sub_name, overwrite
                )
...
def sort_spikes(spikes, folder, overwrite=False, sign="pos"):
    ...
    sorted_idx, match_idx = iterative_sorter(
        all_features, spikes, n_iterations, folder, overwrite=overwrite
    )
...
def main(data_fname, session_fname, sign, overwrite=False):
    ...
    sort_idx, match_idx, artifact_ids = sort_spikes(
        spikes, session.session_dir, overwrite=overwrite, sign=sign
    )
...
def argument_parser():
    ...
    if args.jobs is None:
        if None in (args.datafile, args.sessions):
            raise ArgumentError(
                args.jobs, "Specify either jobs or datafile and sessions"
            )
    ...
    print("Starting {} jobs with {} workers".format(len(joblist), n_cores))
