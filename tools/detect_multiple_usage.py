...
def main(fname_sort_pos, fname_sort_neg, fname_logfile, fname_channels):
    ...
    for name, sign in zip((fname_sort_pos, fname_sort_neg), ("pos", "neg")):
        with open(name, "r") as fid:
            job_channels[sign] = [
                int(os.path.basename(line.strip())[8:-3]) for line in fid.readlines()
            ]

    used_refs = set()
    for chan in job_channels["neg"]:
        print(
            "{} (CSC{}) is referenced to {}".format(
                int_names[chan], chan, name_refs[int_names[chan]]
            )
        )
        used_refs.add(name_refs[int_names[chan]])

    print(
        "The {} channels in {} use the following {} references: {}".format(
            len(job_channels["neg"]), fname_sort_neg, len(used_refs), sorted(used_refs)
        )
    )
...
if __name__ == "__main__":
    ...
    main(fname_pos_channels, fname_neg_channels, fname_logfile, fname_channel_names)
