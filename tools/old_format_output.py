...
def convert_to_mat(groups, outfname, drop_artifacts=False):
    ...
    spikesdict = {"spikes": all_spikes, "index_ts": all_times}

    spikes_fname = outfname + "_spikes.mat"

    savemat(spikes_fname, spikesdict)

    timesdict = {"spikes": all_spikes, "cluster_class": cluster_class}
...
def parse_args():
    ...
    if do_info:
        ...
        writer.writerow(["# Session:  {}, Converted: {}".format(session_name, date)])
        writer.writerow(
            [
        ...
        writer.writerow(
            ["# For Unassigned, 1 means they exist, 0 means they " "do not exist."]
        )
        writer.writerow(
            [
